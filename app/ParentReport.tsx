...
export function ParentReport({ progress, onClose }: ParentReportProps) {
    ...
    <section className="modal-backdrop parent-report-backdrop" role="dialog" aria-modal="true" aria-label="家长学习报告">
      <div className="parent-report-card modal-card">
        ...
        <header className="parent-report-heading">
          <span>家长查看 · 本机数据</span>
          <h2>安琪的学习小报告</h2>
          <p>最近 {recent.length} 次练习的趋势，只看成长，不做同伴排名。</p>
        </header>

        <div className="parent-metrics">
          <span><small>平均准确率</small><strong>{average(recent.map((session) => session.accuracy))}<i>%</i></strong><em>建议稳定在 85% 以上再追求速度</em></span>
          <span><small>平均速度</small><strong>{average(recent.map((session) => session.wpm))}<i> WPM</i></strong><em>速度会随正确指法自然提高</em></span>
        ...
        <footer><span>🔒 所有学习数据只保存在这台设备，不上传、不展示排行榜。</span><button className="modal-primary" onClick={onClose}>返回花园</button></footer>
      </div>
    </section>
  );
}
